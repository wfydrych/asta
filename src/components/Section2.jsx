import React from 'react'
import './Section2.sass'

const Section2 = () => {
  return (
    <div className='section2'>
      <div className='section2__title'>MODULARE TREPPEN - ABMESSUNGEN</div>
      <div className='section2__content'>
        Wählen Sie aus 5 Geländertypen, 4 Treppentypen und 14 Konfigurationen von Podesten und Abdeckungen, 
        um den Raum zu optimieren und das Design an die Inneneinrichtung anzupassen. 
        Prüfen Sie die verfügbaren Optionen und Abmessungen der Modultreppe oder rufen Sie unsere Fachleute an, 
        die Ihnen bei der Auswahl des richtigen Modells für Ihre räumlichen Gegebenheiten helfen werden.
      </div>
      <div className='section2__bg'>
        <div className='section2__bg__btn'>
          <a href="https://www.intertlc.de/Download/Preisliste_Modultreppe_Asta.pdf" target="_blank" rel="noreferrer">
            PREISLISTE
          </a>
        </div>
      </div>
    </div>
  )
}

export default Section2